...
export function SearchSelect({ value, onChange, options, label, disabled = false }: {
  ...
  const results = searching
    ? options.filter(o => matchesSearch(query, [o.label, o.detail, ...o.search]))
    : [];
  ...
  return (
    <div className="relative min-w-0" onBlur={e => {
      ...
      <svg aria-hidden="true" className="absolute left-3 top-3.5 pointer-events-none text-[#86868b]"
        width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <circle cx="10.5" cy="10.5" r="7.5" /><path d="m16 16 5 5" />
      </svg>
      <input
        role="combobox" aria-label={label} aria-expanded={searching} aria-controls={searching ? `${id}-list` : undefined}
        aria-autocomplete="list" aria-activedescendant={searching && active >= 0 && results[active] ? `${id}-${active}` : undefined}
        autoComplete="off" autoCorrect="off" spellCheck={false} disabled={disabled}
        placeholder={label} value={open ? query : selected?.label ?? ""}
        ...
        onKeyDown={e => {
          ...
          if (e.key === "Enter") {
            e.preventDefault();
            if (searching && results[active >= 0 ? active : 0]) choose(results[active >= 0 ? active : 0].id);
          }
        }}
        className="w-full h-11 border border-[#e5e5e7] rounded-lg pl-9 pr-11 bg-white text-[14px] outline-none focus:border-[#0071e3] disabled:bg-[#fafafc] disabled:text-[#6e6e73]"
      />
